...
namespace GoL
{
    class PurchaseHouseScreen:GameScreen
    {
        #region Fields
        string msg;
        Texture2D gradient;
        ...
        public event EventHandler<PlayerIndexEventArgs> FirstHouseSelected;
        public event EventHandler<PlayerIndexEventArgs> SecondHouseSelected;
        public event EventHandler<PlayerIndexEventArgs> ThirdHouseSelected;
        public event EventHandler<PlayerIndexEventArgs> NoHouseSelected;
        #endregion

        #region Initializers
        public PurchaseHouseScreen(string message)
            : this(message, true)
        {
        }

        public PurchaseHouseScreen(string message, bool includeUsageText)
        {
        ...
        public override void HandleInput(InputState input)
        {
            ...
            {
                if (FirstHouseSelected != null)
                {
                    FirstHouseSelected(this, new PlayerIndexEventArgs(playerIndex));
            ...
            {
                if (SecondHouseSelected != null)
                {
                    SecondHouseSelected(this, new PlayerIndexEventArgs(playerIndex));
                }
                ScreenExit();
            }
            else if (input.isNewKeyPress(Keys.D3, ControllingPlayer, out playerIndex))
            {
                if (ThirdHouseSelected != null)
                {
                    ThirdHouseSelected(this, new PlayerIndexEventArgs(playerIndex));
                }
                ScreenExit();
            }
            else if (input.isNewKeyPress(Keys.D4, ControllingPlayer, out playerIndex))
            {
                if (NoHouseSelected != null)
                {
                    NoHouseSelected(this, new PlayerIndexEventArgs(playerIndex));
                }
                ScreenExit();
            }
        }
