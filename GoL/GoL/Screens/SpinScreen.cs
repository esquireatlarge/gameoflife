...
namespace GoL
{
    class SpinScreen:GameScreen
    {
        ...
        public event EventHandler<PlayerIndexEventArgs> Spin;
        #endregion

                #region Initializers
        public SpinScreen(string message)
            : this(message, true)
        {
        }

        public SpinScreen(string message, bool includeUsageText)
        {
            ...
            IsPopup = true;
            TransitioningOnTime = TimeSpan.FromSeconds(0.01);
            TransitioningOffTime = TimeSpan.FromSeconds(0.01);
        }
        ...
        public override void HandleInput(InputState input)
        {
            ...
            if (input.isNewKeyPress(Keys.S, ControllingPlayer, out playerIndex))
            {
                if (Spin != null)
                {
                    Spin(this, new PlayerIndexEventArgs(playerIndex));
                }
                ScreenExit();
            }
        }
