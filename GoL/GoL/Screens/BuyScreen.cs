...
namespace GoL
{
    class BuyScreen:GameScreen
    {
        ...
        public event EventHandler<PlayerIndexEventArgs> BuyStock;
        public event EventHandler<PlayerIndexEventArgs> BuyInsurance;
        #endregion

        
        #region Initializers
        public BuyScreen(string message)
            : this(message, true)
        {
        }

        public BuyScreen(string message, bool includeUsageText)
        {
        ...
        public override void HandleInput(InputState input)
        {
            ...
            if (input.isNewKeyPress(Keys.D1, ControllingPlayer, out playerIndex))
            {
                ...
                {
                    BuyInsurance(this, new PlayerIndexEventArgs(playerIndex));
                }
                ScreenExit();
            }
            else if (input.isNewKeyPress(Keys.D2, ControllingPlayer, out playerIndex))
            {
                if (BuyStock != null)
                {
                    BuyStock(this, new PlayerIndexEventArgs(playerIndex));
                }
                ScreenExit();
            }
        }
